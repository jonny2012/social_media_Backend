...
class CommentController {
    async createComment(
        ...
        try {
            ...
            if (!newComment) {
                ...
                return res.json({ message: "Error on cresting comment" });
            }
    ...
    async getAllPostComments(req: Request, res: Response, next: NextFunction): Promise<any> {
        ...
        try {
            ...
            if (!post) {
                next(ApiError.badRequest("Post with whis postId not found"))
                return res.status(404).json({ message: "Post with whis postId not found" })
            }
